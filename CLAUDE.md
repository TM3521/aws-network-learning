...
const QUIZ = [
  {
    ...
    exp: string,    // 解説（HTML可）
    genre: 'aws'|'net'|'mixed'  // ジャンル（AWSのみ / ネットワークのみ / 複合）
  }
]
```
...
**ジャンル別問題数：** aws=20問 / net=10問 / mixed=5問（合計35問）

| genre | 分類基準 |
|-------|---------|
| `aws` | AWSサービス固有の知識のみで解ける問題 |
| `net` | 一般的なネットワーク概念のみで解ける問題 |
| `mixed` | AWSとネットワーク両方の知識が必要な問題 |
