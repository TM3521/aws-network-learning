...
// --- クイズ（genre: 'aws'=AWSのみ / 'net'=ネットワークのみ / 'mixed'=複合） ---
const QUIZ = [
  {q:'OSIモデルでIPアドレスを使ってルーティングを行う層は？',opts:['第2層（データリンク層）','第3層（ネットワーク層）','第4層（トランスポート層）','第5層（セッション層）'],ans:1,exp:'<strong>第3層（ネットワーク層）</strong>がIPアドレスで異なるネットワーク間のルーティングを担います。',genre:'net'},
  {q:'TCPより UDPが適しているユースケースは？',opts:['ファイルのダウンロード','メール送信','リアルタイム動画配信','Webページ閲覧'],ans:2,exp:'<span class="term" data-term="udp">UDP</span>はコネクションレスで低遅延。リアルタイム動画・音声・ゲームに適します。',genre:'net'},
  {q:'Amazon S3の耐久性として正しいものは？',opts:['99.9%','99.99%','99.999999999%（11ナイン）','100%'],ans:2,exp:'S3は<strong>99.999999999%（イレブンナイン）</strong>の耐久性。複数AZへの自動複製で実現しています。',genre:'aws'},
  {q:'プライベートサブネットからインターネットへのアウトバウンド通信に使うサービスは？',opts:['Internet Gateway','VPC Peering','NAT Gateway','Direct Connect'],ans:2,exp:'<strong>NAT Gateway</strong>がアウトバウンド通信を可能にします。インバウンドは遮断されます。',genre:'mixed'},
  {q:'IAMの「最小権限の原則」とは？',opts:['全ユーザーに同じ権限を付与','必要最小限の権限だけ付与','管理者のみ全権限を持つ','権限は自動的に付与される'],ans:1,exp:'ユーザー・ロールに<strong>業務上必要な最小限の権限だけ付与</strong>する原則。侵害時の被害を最小化します。',genre:'aws'},
  {q:'セキュリティグループとNACLの違いとして正しいものは？',opts:['SGはサブネット・NACLはインスタンスに適用','SGはステートフル・NACLはステートレス','NACLは許可ルールのみ設定できる','両者は全く同じ機能'],ans:1,exp:'<strong>SG</strong>はステートフル・インスタンスレベル。<strong>NACL</strong>はステートレス・サブネットレベルで拒否ルールも設定可能。',genre:'mixed'},
  {q:'DNSの主な役割は？',opts:['MACアドレスをIPに変換','ドメイン名をIPアドレスに変換','IPアドレスを物理アドレスに変換','プライベートIPをパブリックIPに変換'],ans:1,exp:'<span class="term" data-term="dns">DNS</span>はドメイン名（例：example.com）をIPアドレスに変換するシステムです。',genre:'net'},
  {q:'AWS Lambdaの特徴として正しいものは？',opts:['常時起動サーバーが必要','サーバー管理不要でイベント駆動実行','最大24時間継続実行できる','EC2と同じ課金体系'],ans:1,exp:'<strong>Lambda</strong>はサーバーレス。イベント（S3・API等）をトリガーに実行。最大15分・実行時間×リクエスト数で課金。',genre:'aws'},
  {q:'CloudFrontが提供する主な機能は？',opts:['リレーショナルDB','CDN（コンテンツ配信ネットワーク）','VPN','オブジェクトストレージ'],ans:1,exp:'<strong>CloudFront</strong>はCDN。世界中のエッジロケーションにキャッシュし低レイテンシーを実現。',genre:'aws'},
  {q:'RDSのマルチAZ配置の主な目的は？',opts:['コスト削減','高可用性（HA）の確保','読み取りパフォーマンス向上','ストレージ拡張'],ans:1,exp:'<strong>マルチAZ</strong>は別AZに同期レプリカを保持し、障害時に自動フェイルオーバーします。',genre:'aws'},
  {q:'10.0.0.0/24 で使用可能なホストアドレス数は？',opts:['128','254','512','1024'],ans:1,exp:'<strong>/24</strong>はホスト部8bit。2⁸=256のうちネットワークアドレス・ブロードキャストを除いた<strong>254個</strong>が使用可能。',genre:'net'},
  {q:'CloudTrailの主な用途は？',opts:['パフォーマンスメトリクス監視','APIコールの記録と監査ログ','アプリケーションログ収集','ネットワークトラフィック分析'],ans:1,exp:'<strong>CloudTrail</strong>はAPIコールを記録。誰がいつ何を操作したかの監査ログをコンプライアンス対応に活用。',genre:'aws'},
  {q:'NATの主な目的は？',opts:['IPをMACアドレスに変換','プライベートIPとパブリックIPを変換','ドメイン名をIPに変換','ポート番号をサービス名に変換'],ans:1,exp:'<strong>NAT</strong>はプライベートIPをパブリックIPに変換。複数ホストが1つのグローバルIPを共有できます。',genre:'net'},
  {q:'Route 53でDNSフェイルオーバーに使う機能は？',opts:['エイリアスレコード','ヘルスチェック','ゾーンファイル','TTL設定'],ans:1,exp:'<strong>Route 53のヘルスチェック</strong>でエンドポイントを監視し、障害時に自動フェイルオーバーします。',genre:'aws'},
  {q:'AWS KMSが主に提供する機能は？',opts:['ユーザー認証','暗号化キーの管理','ログの保存','ネットワーク監視'],ans:1,exp:'<strong>KMS</strong>は暗号化キー（CMK）の作成・管理・使用を行うマネージドサービス。S3・EBS・RDS等と統合。',genre:'aws'},
  {q:'スポットインスタンスの特徴は？',opts:['常に固定価格','余剰リソース活用で最大90%割引だが中断あり','1〜3年予約で割引','リザーブドより高コスト'],ans:1,exp:'<strong>スポットインスタンス</strong>はAWS余剰リソースを活用し最大90%割引。AWSが必要な場合2分前通知で中断あり。',genre:'aws'},
  {q:'AZとは何か？',opts:['同一リージョン内の独立したデータセンター群','異なる国のサーバーファーム','コンテンツキャッシュの拠点','インターネット接続ポイント'],ans:0,exp:'<span class="term" data-term="az">AZ（アベイラビリティゾーン）</span>は独立した電源・冷却・ネットワークを持つデータセンター群。複数AZで高可用性を実現。',genre:'aws'},
...
const DEV_READ = [
  ...
  `},
  {id:'dr-9',icon:'🪄',title:'⑨ プロキシサーバー',lead:'クライアントとサーバーの「仲介役」として通信を中継する仕組みです',html:`
    <p class="a-p"><span class="term" data-term="proxy">プロキシサーバー</span>は、クライアント（PCやスマホ）とサーバーの間に立ち、通信を代理で中継する仕組みです。直接通信する代わりにプロキシが間に入ることで、キャッシュ・フィルタリング・セキュリティ向上など様々な機能を追加できます。</p>
    <div class="analogy"><div class="analogy-lbl">💡 わかりやすい例え</div>プロキシは「代理人」です。外国（インターネット）との交渉を本人（PC）の代わりに代理人（プロキシ）が行います。代理人が交渉メモをとっておけば（キャッシュ）、次の同じ質問には本国に問い合わせずに即答できます。</div>

    <div class="a-h3">フォワードプロキシ（通常のプロキシ）</div>
    <p class="a-p">クライアント側に設置され、社内のPCがインターネットへ出る通信を代理するプロキシです。会社や学校のネットワークでよく使われます。</p>
    <div class="steps">
      <div class="step"><div class="step-n">1</div><div class="step-body"><div class="step-t">クライアントがプロキシにリクエスト</div><div class="step-d">PCはWebサーバーに直接ではなく、プロキシサーバーに「example.comを取ってきて」とリクエストする</div></div></div>
      <div class="step"><div class="step-n">2</div><div class="step-body"><div class="step-t">プロキシが代わりに取得</div><div class="step-d">プロキシがWebサーバーにアクセスしてコンテンツを取得する。Webサーバーにはプロキシのアドレスが見え、クライアントのIPは隠れる</div></div></div>
      <div class="step"><div class="step-n">3</div><div class="step-body"><div class="step-t">キャッシュして返す</div><div class="step-d">取得したコンテンツをプロキシに保存（キャッシュ）し、クライアントに返す。次回同じリクエストはキャッシュから即答</div></div></div>
    </div>

    <div class="a-h3">フォワードプロキシの主な用途</div>
    <table class="tbl"><tr><th>用途</th><th>説明</th></tr>
    <tr><td>🗂️ キャッシュ</td><td>よくアクセスされるページをプロキシに保存し、外部への通信を減らして高速化</td></tr>
    <tr><td>🔒 フィルタリング</td><td>会社・学校で特定のサイトへのアクセスを制限。有害サイトや業務外サイトをブロック</td></tr>
    <tr><td>🕵️ 匿名性</td><td>サーバー側にクライアントの本当のIPアドレスを隠せる（プロキシのIPが見える）</td></tr>
    <tr><td>📋 アクセスログ</td><td>誰がどのサイトにいつアクセスしたか記録・監査できる</td></tr></table>

    <div class="a-h3">リバースプロキシ</div>
    <p class="a-p">フォワードプロキシとは逆に、<strong>サーバー側に置く</strong>プロキシです。インターネットからのリクエストを受け取り、後ろの本物のサーバーに転送します。クライアント（ユーザー）からは、リバースプロキシが「本物のサーバー」に見えます。</p>
    <div class="analogy"><div class="analogy-lbl">💡 わかりやすい例え</div>企業の「受付」と同じ。来客（ユーザー）は受付（リバースプロキシ）に用件を伝え、受付が適切な担当者（バックエンドサーバー）に案内します。来客は担当者が誰かを知らず、担当者も来客の詳細な素性を知りません。</div>

    <div class="a-h3">リバースプロキシの主な用途</div>
    <table class="tbl"><tr><th>用途</th><th>説明</th></tr>
    <tr><td>⚖️ 負荷分散</td><td>複数のバックエンドサーバーにリクエストを振り分ける（AWSのALBが相当）</td></tr>
    <tr><td>🔒 SSLターミネーション</td><td>HTTPS の暗号化・復号をリバースプロキシが引き受け、バックエンドは HTTP で通信できる</td></tr>
    <tr><td>🗂️ コンテンツキャッシュ</td><td>静的ファイルをリバースプロキシでキャッシュしてバックエンドの負荷を軽減（CDNの仕組み）</td></tr>
    <tr><td>🛡️ セキュリティ</td><td>バックエンドサーバーのIPや構成を隠蔽。WAFと組み合わせて攻撃を遮断</td></tr></table>

    <div class="a-h3">フォワードプロキシ vs リバースプロキシ</div>
    <table class="tbl"><tr><th></th><th>フォワードプロキシ</th><th>リバースプロキシ</th></tr>
    <tr><td>設置場所</td><td>クライアント側（社内ネットワーク内）</td><td>サーバー側（データセンター内）</td></tr>
    <tr><td>誰のために</td><td>クライアント（社員・学生）のため</td><td>サーバー（サービス運営者）のため</td></tr>
    <tr><td>何を隠す</td><td>クライアントのIPをサーバーに隠す</td><td>バックエンドサーバーの構成をクライアントに隠す</td></tr>
    <tr><td>AWS相当サービス</td><td>NAT Gateway（送信元を隠す）</td><td>ALB / CloudFront（受信側を抽象化）</td></tr></table>

    <div class="tip"><div class="tip-lbl">☁️ AWSとプロキシの関係</div>
    AWSサービスの多くはリバースプロキシの考え方で設計されています。<br>
    <strong>ALB（Application Load Balancer）</strong>はリバースプロキシとして複数のEC2に負荷分散し、SSLターミネーションも担います。<br>
    <strong>CloudFront</strong>はリバースプロキシ型のCDNで、エッジロケーションでコンテンツをキャッシュしてオリジンの負荷を軽減します。<br>
